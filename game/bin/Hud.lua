...
function Hud:new()
    ...
    inst.defaultW, inst.defaultH = love.graphics.getDimensions()
    inst.scale = 1
    inst.scaleX = 1
    inst.scaleY = 1
    inst.w, inst.h = inst.defaultW / inst.scale, inst.defaultH / inst.scale

    local font = lg.newImageFont("assets/fonts/default.png",
        "ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz0123456789#.,!?: ]['%/=+ö?", 2)
    lg.setFont(font)
...
function Hud.drawScreen(self)
    ...
    -- local scale = math.
    self:scaleCoords(scale, scale)
    -- print(("Dimensions: %2ix%2i and Screen size: %2ix%2i"):format(w, h, Screen.width, Screen.height))
    -- print(("Scale: %.2fx%.2f"):format(self.scaleX, self.scaleY))
    -- print(("Screen.canvas size: %2ix%2i"):format(Screen.canvas:getWidth(), Screen.canvas:getHeight()))
    lg.setColor(1, 1, 1)
    lg.setShader(shader_screen)
    lg.draw(Screen.canvas, (w - Screen.canvas:getWidth()) * 0.5, (h - Screen.canvas:getHeight()) * 0.5)
    lg.setShader()
end

function Hud.applyHudDrawingSettings(self)
    self:scaleCoords(Screen.width / self.defaultW * self.scale, Screen.height / self.defaultH * self.scale)
end
